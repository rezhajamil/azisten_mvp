...
@section('content')
    ...
    <div class="card-body">
      <table id="tableCariKos" class="table table-bordered table-striped">
        ...
        <tbody>
          @foreach ($alat_kos_purchases as $key=>$data)
              <tr>
                <td>{{ $key+1 }}</td>
                ...
                <td>
                  ...
                  <form action="{{ route('admin.pesan_alat_kos.destroy',$data->id) }}" method="post">
                    @method('delete')
                    @csrf
              ...
              <x-change_status_modal id="{{ $data->id }}" desc="{{ $data->description }}" service="pesan_alat_kos"  value="{{ $data->status_id }}"></x-change_status_modal>
          @endforeach
        </tbody>
      </table>
    </div>
